...
以下の問いが、どの思考モードに該当するか判定してください。

【問い】
${parsed.prompt}

【思考モード定義】
- モード1：状況と言葉の整理（定義、語の意味、事実、前提などの明確化）
- モード2：立場・視点の切替（関係者や他者の視点へのジャンプ）
- モード3：構造と因果の解剖（要素分解、因果分析、構造整理）
- モード4：目的と価値の再設計（根本的な問い直し、前提崩し、意味変容）
...
`.trim();
    ...
    try {
      ...
      console.warn("❓ GPTモード判定のJSON解析に失敗:", judge.choices[0].message.content);
    }
